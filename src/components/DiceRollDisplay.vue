<template>
    ...
    <div class="fixed bottom-24 right-8 z-50" v-if="diceRollsVisible">
        <!-- Roll History Stack -->
        <div class="space-y-2 flex flex-col items-end">
            ...
            <div v-if="lastDiceRolls.length > 0" class="bg-base-300 shadow-lg rounded-lg max-w-80">
                <div class="p-4">
                    <div class="text-xs opacity-70 mb-1">
                        {{ lastDiceRolls[0].monster?.name || 'Unknown' }}
                    </div>
                    <div class="flex items-center justify-start gap-3">
...
const formatRollNotation = (roll) => {
    let notation = roll.originalNotation || roll.notation
    if (roll.rollMode === 'advantage') {
        notation += ' (ADV)'
    } else if (roll.rollMode === 'disadvantage') {
        notation += ' (DIS)'
    } else if (roll.rollMode === 'crit') {
        notation += ' (CRIT)'
    }
    return notation
}
